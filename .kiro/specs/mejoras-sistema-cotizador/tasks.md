...
- [x] 1. Seed de configuración IA en base de datos
  ...
  - [x] 1.1 Crear script `backend/scripts/seed-config-ia.js`
...
- [x] 2. Req. 1 — Recuperación de cuenta por teléfono (backend)
  - Implementar el endpoint `POST /api/auth/recuperar-por-telefono` con respuesta genérica anti-enumeración.
  - _Requisitos: 1.1, 1.2, 1.3, 1.4, 1.5_

  - [x] 2.1 Implementar `servicioAuth.recuperarPorTelefono(telefono)`
  ...
  - [x] 2.2 Agregar endpoint `POST /api/auth/recuperar-por-telefono` en `backend/src/rutas/auth.js`
  ...
  - [x] 2.3 Escribir tests de propiedades para recuperación por teléfono
...
- [x] 3. Req. 1 — Recuperación por teléfono (frontend)
  - Agregar pestaña "Número de teléfono" en la página de recuperación de contraseña.
  - _Requisitos: 1.6, 1.7, 1.8_

  - [x] 3.1 Modificar `frontend/src/paginas/RecuperarContrasena.jsx`
...
- [x] 4. Req. 2 — Configuración de modo y modelos de IA (backend)
  ...
  - [x] 4.1 Crear `backend/src/asistente/servicioConfigIA.js`
  ...
  - [x] 4.2 Implementar `obtenerModelosIA` y `actualizarModelosIA` en `controladorConfiguracion.js`
  ...
  - [x] 4.3 Agregar rutas `GET/PUT /api/configuracion/modelos-ia` en `backend/src/rutas/configuracion.js`
    - Proteger ambas rutas con `verificarTokenAdmin`.
    - _Requisitos: 2.1, 2.2_

  - [x] 4.4 Integrar `servicioConfigIA` en `controladorAsistente.js`
  ...
  - [x] 4.5 Actualizar `servicioLLM.js` para recibir modelos como parámetros
  ...
  - [x] 4.6 Actualizar `orquestadorAgentes.js` para recibir `configIA`
  ...
  - [x] 4.7 Actualizar `agenteClasificador.js` para recibir modelo como parámetro
  ...
  - [x] 4.8 Escribir tests de propiedades para configuración IA
...
- [x] 5. Req. 2 — Configuración de modo y modelos de IA (frontend)
  ...
  - [x] 5.1 Modificar `frontend/src/paginas/AdminConfiguracion.jsx`
...
- [x] 6. Req. 3 — Persistencia de selección de componentes
  - Crear hook `usePersistenciaSeleccion.js` e integrarlo en `Cotizador.jsx`.
  - _Requisitos: 3.1, 3.2, 3.3, 3.4, 3.5, 3.6, 3.7_

  - [x] 6.1 Crear `frontend/src/hooks/usePersistenciaSeleccion.js`
  ...
  - [x] 6.2 Integrar `usePersistenciaSeleccion` en `frontend/src/paginas/Cotizador.jsx`
  ...
  - [x] 6.3 Escribir tests para persistencia de selección
...
- [x] 7. Req. 4 — Corrección del logo en PDF comercial
  - Modificar `servicioPDF.js` para buscar el logo en múltiples rutas candidatas.
  - _Requisitos: 4.1, 4.2, 4.3, 4.4, 4.5_

  - [x] 7.1 Modificar `backend/src/servicios/servicioPDF.js`
...
- [x] 8. Req. 5 — Corrección del botón PDF en cotización vencida
  - Cambiar la descarga de PDF de `window.open` a fetch programático para interceptar errores HTTP.
  - _Requisitos: 5.1, 5.2, 5.3, 5.4, 5.5, 5.6_

  - [x] 8.1 Modificar función de descarga PDF en `frontend/src/paginas/HistorialCliente.jsx`
...
- [x] 9. Req. 6 — Mejora del diseño del reporte Excel
  - Actualizar `servicioExcel.js` con estilos profesionales.
  - _Requisitos: 6.1, 6.2, 6.3, 6.4, 6.5, 6.6, 6.7, 6.8_

  - [x] 9.1 Modificar `backend/src/servicios/servicioExcel.js`
...
- [x] 10. Req. 7 — Estado "Vencida" en historial de cotizaciones
  - Cambio puntual en la capa de presentación del historial.
  - _Requisitos: 7.1, 7.2, 7.3, 7.4, 7.5_

  - [x] 10.1 Modificar `frontend/src/paginas/HistorialCliente.jsx`
...
- [x] 12. Req. 9 — Notificar equipo listo (backend)
  ...
  - [x] 12.1 Agregar función `notificarEquipoListo` en `backend/src/servicios/servicioNotificaciones.js`
  ...
  - [x] 12.2 Implementar `notificarCotizacionLista` en `controladorCotizaciones.js`
...
- [x] 13. Req. 9 — Notificar equipo listo (frontend)
  - Conectar el botón "Notificar equipo listo" en `/validar` con el endpoint real.
  - _Requisitos: 9.7, 9.8_

  - [x] 13.1 Modificar `frontend/src/paginas/ValidadorCotizaciones.jsx`
...
- [x] 14. Req. 10 — Filtros de ordenamiento por precio en el cotizador
  ...
  - [x] 14.1 Implementar función `ordenarProductos` en `frontend/src/paginas/Cotizador.jsx`
  ...
  - [x] 14.2 Agregar estado y control de UI en `frontend/src/paginas/Cotizador.jsx`
  ...
  - [x] 14.3 Escribir tests de propiedades para ordenamiento de precio
...
- [x] 15. Checkpoint final — Verificación end-to-end
  - Verificar que todos los cambios funcionan de punta a punta sin romper flujos existentes.
  - _Requisitos: Todos_

  - [x] 15.1 Verificar build de backend y frontend
  ...
  - [x] 15.2 Ejecutar tests existentes para detectar regresiones
  ...
  - [x] 15.3 Ejecutar tests de propiedades nuevos
...
- [x] 16. Req. 11 — Configuración de claves API de IA desde panel admin
  ...
  - [x] 16.1 Extender `servicioConfigIA.js` para leer y desencriptar API keys desde BD
  ...
  - [x] 16.2 Implementar `obtenerApiKeysIA` y `actualizarApiKeysIA` en `controladorConfiguracion.js`
  ...
  - [x] 16.3 Agregar rutas `GET/PUT /api/configuracion/api-keys-ia` en `configuracion.js`
    - Proteger ambas rutas con `verificarTokenAdmin`.
    - _Requisitos: 11.1, 11.2_

  - [x] 16.4 Propagar `nvidia_api_key` de `configIA` a `agenteClasificador.js`
  ...
  - [x] 16.5 Propagar `nvidia_api_key` de `configIA` a `servicioEmbeddings.js`
  ...
  - [x] 16.6 Actualizar `servicioLLM.js` para usar API keys de `configIA`
  ...
  - [x] 16.7 Agregar campos de clave API en `AdminConfiguracion.jsx`
